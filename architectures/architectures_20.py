...
def flownet_s(imgs_0, imgs_1, flows):
    ...
    with slim.arg_scope([slim.conv2d],
			...
			activation_fn=tf.nn.elu): #TODO check
    ...
    for i in range(4):
        ...
        with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
            ...
            flow_up = slim.conv2d_transpose(
                flow_predict, 2, [4, 4], 2, scope='flow_up_' + str(6 - i) + "_to_" + str(5 - i))*20
        ...
        # get old convolution
        to_concat = tf.get_default_graph().get_tensor_by_name('conv' + str(5 - i) + "_1/Elu:0")
    ...
    flow_up = tf.image.resize_bilinear(flow_predict, FLAGS.img_shape[:2])*20
