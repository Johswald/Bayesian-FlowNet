...
def add_gt(flow, calc_flow, b):
	...
	Keyword arguments:
	flow -- groundtruth flow
	calc_flows -- predicted flow
    b -- indicating of groundtruth is available (b == 1) or not
    """
    def _add_gt(flow, calc_flow, b):
        ...
        scale_flow[:, :, 0] = u * \
            float(FLAGS.img_shape[1]) / FLAGS.d_shape_img[1]
        scale_flow[:, :, 1] = v * \
            float(FLAGS.img_shape[0]) / FLAGS.d_shape_img[0]
        gt_flow[:, :, 0] = u_gt
        gt_flow[:, :, 1] = v_gt
